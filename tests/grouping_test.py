...
from .context import _Add, _Eq, _Mul, assert_equal, x, y


theta = Symbol('theta', real=True, positive=True)


func_arg_examples = [('\\int ', 'x\\differentialD x', Integral(x, x)), ('\\sin', '\\theta ', sin(theta))]

example_groups = [('1+2', '3-4', _Mul(_Add(1, 2), _Add(3, _Mul(-1, 4))))]

example_relation_lists = [('x=1', 'y=2', [_Eq(x, 1), _Eq(y, 2)])]
...
delimiter_pairs = {'{': '}', **func_modifiable_delimiter_pairs}
...
def test_func_arg_groupings(func, args, output):
    ...
    for left, right in func_modifiable_delimiter_pairs.items():
        ...
        assert_equal(
            "{func}\\left{left}{args}\\right{right}".format(left=left, right=right, func=func, args=args), output
        )
        assert_equal(
            "{func}\\mleft{left}{args}\\mright{right}".format(left=left, right=right, func=func, args=args), output
        )
...
def test_delimiter_groupings(group1, group2, output):
    for left, right in delimiter_pairs.items():
        assert_equal(
            "{left}{group1}{right}{left}{group2}{right}".format(left=left, right=right, group1=group1, group2=group2),
            output,
        )
        assert_equal(
...
def test_delimiter_relation_lists(group1, group2, output):
    for left, right in delimiter_pairs.items():
        assert_equal(
            "{left}{group1},{group2}{right}".format(left=left, right=right, group1=group1, group2=group2), output
        )
        assert_equal(
            "{left}{group1};{group2}{right}".format(left=left, right=right, group1=group1, group2=group2), output
        )
        assert_equal(
            "\\left{left}{group1},{group2}\\right{right}".format(left=left, right=right, group1=group1, group2=group2),
            output,
        )
        assert_equal(
            "\\left{left}{group1};{group2}\\right{right}".format(left=left, right=right, group1=group1, group2=group2),
        ...
        assert_equal(
            "\\mleft{left}{group1},{group2}\\mright{right}".format(
                left=left, right=right, group1=group1, group2=group2
            ),
            output,
        )
