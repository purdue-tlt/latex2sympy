...
from sympy import Add, Eq, exp, Mul, pi, Pow, Rational, sqrt, srepr, Symbol
...
def get_min_max_examples(cmd, func):
    return [
        ...
        (
            f"{cmd}(\\frac{{-4}}{{3}}, \\frac{{-2}}{{1}}, \\frac{{0}}{{9}}, -3)",
            func(Rational(-4, 3), Rational(-2, 1), Rational(0, 9), -3),
        ),
        ...
        (
            f"{cmd}(\\frac{{274}}{{5+2}}, \\exp(12.4), 1.4E2)",
            func(_Mul(_Pow(_Add(5, 2), -1), 274), exp(Rational('12.4')), Rational('1.4E2')),
        ),
        (f"{cmd}(x)", func(x)),
        (f"{cmd}(x, y)", func(x, y)),
        (f"{cmd}(y, x)", func(y, x)),
        ...
        (
            f"{cmd}(\\pi,12E2,84,\\sqrt{{5}},\\frac{{12}}{{5}})",
            func(pi, Rational('12E2'), 84, sqrt(5), Rational(12, 5)),
        ),
...
def assert_equal(latex, expr, variable_values=None):
